...
function Rush() {
    ...
    this.timelimit = 170; //In seconds. 300 = 5 minutes
    this.packetLB = 48;
    this.state;
...
Rush.prototype.formatTime = function (time) { //300
    ...
    if (sec < 10) return min + ":0" + Math.round(sec) + ""
    if (Math.floor(sec) == 60) return min + 1 + ":00"
    else return min + ":" +  Math.floor(sec , 1) + ""
};
